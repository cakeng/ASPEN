...
char *read_check_and_return (FILE *fp, char *buffer, char *check_str, unsigned int *line_num)
{
    ...
    {
        ERROR_PRTF ("ASPEN DNN file parse error: Unexpected EOF.");
        return NULL;
    }
    (*line_num)++;
...
void apu_load_dnn_data_from_file (aspen_dnn_t *dnn, char *input_path)
{
    ...
    if (fp == NULL)
    {
        ERROR_PRTF ("ASPEN DNN file %s not found.", input_path);
        return;
    }
    ...
    while (feof(fp) == 0)
    {
        ...
        if (buffer == NULL)
        {
            ERROR_PRTF ("ASPEN DNN file %s parse error: Failed to allocate memory.", input_path);
            free (buffer);
            if (bn_mean != NULL)
                free (bn_mean);
            if (bn_weight != NULL)
        ...
        {
            ERROR_PRTF ("ASPEN DNN file %s parse error: Failed to read data.", input_path);
            free (buffer);
            if (bn_mean != NULL)
                free (bn_mean);
            if (bn_weight != NULL)
...
void apu_save_dnn_to_file(aspen_dnn_t *dnn, char *filename)
{
    ...
    if (fp == NULL)
    {
        ERROR_PRTF ("Error: apu_save_dnn_to_file Failed to open file %s for writing", filename);
        return;
    }
        ...
        for (unsigned int j = 0; j < NUM_TENSORS; j++)
        {
            ...
            *line_num += 1;
            ...
            else
            {
                ...
                if (tensor == NULL)
                {
                    ERROR_PRTF ("ASPEN DNN file %s parse error: Failed to create tensor.", filename);
                    apu_destroy_dnn(dnn);
                    return NULL;
                }
                ...
                if (skip_alloc == 0)
                {
                    ...
                    if (tensor->data == NULL)
                    {
                        ERROR_PRTF ("ASPEN DNN file %s parse error: Failed to allocate tensor data.", filename);
                        apu_destroy_dnn(dnn);
                        return NULL;
                    }
...
aspen_dnn_t *apu_load_dnn_from_file(char *filename)
{
    ...
    if (fp == NULL)
    {
        ERROR_PRTF ("ASPEN DNN file %s not found.", filename);
        return NULL;
    }
    unsigned int line_num = 0;
...
nasm_t *apu_load_nasm_from_file(char *filename, aspen_dnn_t *dnn)
{
    ...
    if (fp == NULL)
    {
        ERROR_PRTF ("ASPEN DNN NASM load error: file %s not found.", filename);
        return NULL;
    }
    char line[MAX_STRING_LEN] = {0};
    char* ptr;
