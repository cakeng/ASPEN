...
void *aspen_dynamic_malloc (size_t num, size_t size)
{
    ...
    {
        ERROR_PRTF ("Error: Requested memory size %ld KiB is too large.", num*size/1024);
        assert (0);
    }
    #endif
    void* ptr = NULL;
...
void* load_arr (char *file_path, unsigned int size)
{
    ...
    if (fptr != NULL)
    {
        ...
        if (num < size)
        {
            ERRNO_PRTF ("Error: Failed to read file %s - Size mismatch. File size: %ld, Req. size: %d. Exiting.", 
                file_path, num, size);
            free (input);
            fclose(fptr);
            assert(0);
    ...
    {
        ERRNO_PRTF ("Error: Failed to open file %s. Exiting.", file_path);
        free (input);
        return NULL;
    }
}
