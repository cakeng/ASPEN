...
dir_name="visual_dse64"

edge_dir="./logs/" + dir_name
server_dir="./logs/" + dir_name
edge_file_name=""
server_file_name=""
...
for iter in range(0, NUM_ITERATIONS):
    ...
    spinn_edge_data = [pd.read_csv(f'{edge_dir}/edge_{edge_id}/{DNN}_conventional_EDGE_{DNN}_B{BATCH}_T{NUM_TILES}_Iter{iter}.csv') for edge_id in range(0, NUM_DEVICE)]
    spinn_server_data = [pd.read_csv(f'{server_dir}/edge_{edge_id}/{DNN}_conventional_SERVER_{DNN}_B{BATCH}_T{NUM_TILES}_Iter{iter}.csv') for edge_id in range(0, NUM_DEVICE)]
    ...
    plt.xlabel("Time (ms)")
    plt.xticks(range(0, int(max_timestamp+10), 50))
    plt.subplot(2, 1, 2)
    plt.xlim((0, max_timestamp+10))
    plt.xlabel("Time (ms)")
    plt.xticks(range(0, int(max_timestamp+10), 50))
