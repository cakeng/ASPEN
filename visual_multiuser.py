...
for dev in range(1, NUM_DEVICE):
    ...
    plt.scatter(rx_log["computed time (ms)"], [-50 for _ in range(len(rx_log))], alpha=VISUAL_ALPHA, linewidths=0, color='orange')
    transfer_count += 5
...
max_timestamp = max(max_timestamp, *seq_data_rx[0]['computed time (ms)'], *seq_data_rx[1]['computed time (ms)'], *seq_data_rx[2]['computed time (ms)'])
