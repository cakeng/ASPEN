...
int main(void)
{
    ...
    nasm_t *bert_nasm = apu_create_transformer_encoder_nasm(bert_dnn, 500e6, 16, 480);
    if (bert_nasm == NULL) 
    {
    ...
    sprintf (nasm_file_name, "data/bert_S%d_B%d_T%d_%2.1e.nasm"
        , bert_nasm->tr_seq_len, bert_nasm->batch_size, MIN_NINST_TILE_PER_LAYER,
        (double)bert_nasm->flop_per_ninst);
    apu_save_nasm_to_file (bert_nasm, nasm_file_name);
    int gpu = 0;
    // aspen_dnn_t *bert_dnn = NULL;
    // nasm_t *bert_nasm = apu_load_nasm_from_file ("data/bert_S480_B1_T10_1.0e+08.nasm", &bert_dnn);
    ...
    input_params[BATCH] = 16; input_params[NUM_SEQ] = 480; input_params[NUM_HIDDEN] = 768;
    ...
    for (int i = 12; i < 13; i++)
    {
        ...
        char filename[256];
        sprintf (filename, "data/BERT_22_transformer_layer_0_output.bin");
