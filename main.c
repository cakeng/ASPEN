...
int main(void)
{
    ...
    aspen_dnn_t *bert_dnn = apu_create_dnn ("data/cfg/bert_large_encoder.cfg", "data/bert_large/bert_large_data.bin");
    // aspen_dnn_t *gpt2_dnn = apu_create_dnn ("data/cfg/bert_large_encoder.cfg", "data/gpt2/bert_large_data.bin");
    // // // if (bert_dnn == NULL) 
    // // // {
    ...
    apu_save_dnn_to_file (bert_dnn, "data/bert_large_base.aspen");
    ...
    nasm_t *bert_nasm = apu_create_transformer_nasm(bert_dnn, 10e6, 100, 8, 128);
    // nasm_t *gpt2_nasm = apu_create_transformer_nasm(gpt2_dnn, 10e6, 100, 1, 1024);
    ...
    sprintf (nasm_file_name, "data/bert_large_S%d_B%d_M%d_%2.1e.nasm"
    ...
    int gpu = -1;
    // aspen_dnn_t *bert_dnn = apu_load_dnn_from_file ("data/bert_large_base.aspen");
    // nasm_t *bert_nasm = apu_load_nasm_from_file ("data/bert_large_S128_B1_M50_1.0e+07.nasm", bert_dnn);
    ...
    rpool_add_nasm (rpool, bert_nasm, 1.0, "data/bert_large/bert_large_text_len_128_embedded_input_batch8.bin");
    ...
    input_params[BATCH] = 8; input_params[NUM_SEQ] = 128; input_params[NUM_HIDDEN] = 1024;
    // input_params[BATCH] = 1; input_params[OUT_C] = 3; input_params[OUT_H] = 416; input_params[OUT_W] = 416;
    void *dog_data = aspen_load_input ("data/bert_large/bert_large_text_len_128_embedded_input_batch8.bin", input_params, sizeof(float));
    ...
    //     char filename[1024];
    //     sprintf (filename, "data/bert_large/bert_large_text_len_128_sequence_output_batch1.bin");
